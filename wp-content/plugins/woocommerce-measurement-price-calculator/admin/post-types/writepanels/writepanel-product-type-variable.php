...
// pre-2.0 support
add_action( 'woocommerce_product_after_variable_attributes_js', 'wc_price_calculator_product_after_variable_attributes_js' );

/**
 * Render our custom product Area/Volume meta fields in the product
 * variation form javascript.  This is for WooCommerce pre-2.0 support
 */
function wc_price_calculator_product_after_variable_attributes_js() {
	?>
	<tr>\
		<td class="hide_if_variation_virtual"><label><?php echo esc_js( __( 'Area', WC_Measurement_Price_Calculator::TEXT_DOMAIN ) ) . ' (' . get_option( 'woocommerce_area_unit' ).'):'; ?> <a class="tips" data-tip="<?php echo esc_js( __( 'Overrides the area calculated from the width/length dimensions for the Measurements Price Calculator.', WC_Measurement_Price_Calculator::TEXT_DOMAIN ) ); ?>" href="#">[?]</a></label><input type="number" size="5" name="variable_area[' + loop + ']" placeholder="0.00" /></td>\
		<td class="hide_if_variation_virtual"><label><?php echo esc_js( __( 'Volume', WC_Measurement_Price_Calculator::TEXT_DOMAIN ) ) . ' (' . get_option( 'woocommerce_volume_unit' ).'):'; ?> <a class="tips" data-tip="<?php echo esc_js( __( 'Overrides the volume calculated from the width/length/height dimensions for the Measurements Price Calculator.', WC_Measurement_Price_Calculator::TEXT_DOMAIN ) ); ?>" href="#">[?]</a></label><input type="number" size="5" name="variable_volume[' + loop + ']" placeholder="0.00" /></td>\
	</tr>\
	<?php
}
