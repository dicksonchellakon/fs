jQuery(document).ready(function($) {
	...
	function addPricingPerUnitLabel(pricingLabel) {
		...
		// ie 'Regular Price ($ / ft)'
		var regularPriceLabel = $("label[for='_regular_price']").text().substr(0, $("label[for='_regular_price']").text().indexOf('(')) + '(' + wc_price_calculator_admin_params['woocommerce_currency_symbol'] + ' / ' + pricingLabel + ')';
		$("label[for='_regular_price']").text(regularPriceLabel);

		// ie 'Sale Price ($ / ft)'
		var salePriceLabel = $("label[for='_sale_price']").text().substr(0, $("label[for='_sale_price']").text().indexOf('(')) + '(' + wc_price_calculator_admin_params['woocommerce_currency_symbol'] + ' / ' + pricingLabel + ')';
		$("label[for='_sale_price']").text(salePriceLabel);
		...
		$('.woocommerce_variable_attributes input[type="text"], .woocommerce_variable_attributes input[type="number"]').each(
			function(index,el) {
				el = $(el);
				if ('variable_price' == el.attr('name').substr(0, 14) || 'variable_regular_price' == el.attr('name').substr(0, 22) || 'variable_sale_price' == el.attr('name').substr(0, 19)) {
					var delimPos = el.prev().text().indexOf('(') > -1 ? el.prev().text().indexOf('(') - 1 : el.prev().text().indexOf(':');
					var priceLabel = el.prev().text().substr(0, delimPos) + ' (' + wc_price_calculator_admin_params['woocommerce_currency_symbol'] + ' / ' + pricingLabel + '):';
		...
		var el = $( '.wc-calculator-pricing-table .measurement-range-column span' );
		el.text( el.data( 'text' ) + ' (' + pricingLabel + ')' );

		el = $( '.wc-calculator-pricing-table .price-per-unit-column span' );
		el.text( el.data( 'text' ) + ' (' + wc_price_calculator_admin_params['woocommerce_currency_symbol'] + ' / ' + pricingLabel + ')' );

		el = $( '.wc-calculator-pricing-table .sale-price-per-unit-column span' );
		el.text( el.data( 'text' ) + ' (' + wc_price_calculator_admin_params['woocommerce_currency_symbol'] + ' / ' + pricingLabel + ')' );
	}
	...
	function removePricingPerUnitLabel() {
		// ie 'Regular Price ($)'
		var regularPriceLabel = $("label[for='_regular_price']").text().substr(0, $("label[for='_regular_price']").text().indexOf('(')) + '(' + wc_price_calculator_admin_params['woocommerce_currency_symbol'] + ')';
		$("label[for='_regular_price']").text(regularPriceLabel);

		// ie 'Sale Price ($)'
		var salePriceLabel = $("label[for='_sale_price']").text().substr(0, $("label[for='_sale_price']").text().indexOf('(')) + '(' + wc_price_calculator_admin_params['woocommerce_currency_symbol'] + ')';
		$("label[for='_sale_price']").text(salePriceLabel);
		...
		$('.woocommerce_variable_attributes input[type="text"]').each(
			function(index,el) {
				el = $(el);
				if ('variable_price' == el.attr('name').substr(0, 14) || 'variable_regular_price' == el.attr('name').substr(0, 22) || 'variable_sale_price' == el.attr('name').substr(0, 19)) {
					if (-1 != el.prev().text().indexOf('(')) {
						var priceLabel = el.prev().text().substr(0, el.prev().text().indexOf('(')) + ':';
						el.prev().text( priceLabel );
					}
				}
			}
	...
	// calculator" mode by adding/removeing the 'per unit' label for regular/sale
	...
	$('._measurement_pricing_calculator_enabled').change(
		function() {
			...
			if ('_measurement_' + $('#_measurement_price_calculator').val() + '_pricing_calculator_enabled' == $( this ).attr( 'id' ) ) {
				...
				$( this ).closest( 'div' ).find( '._measurement_pricing_inventory_enabled' ).change();
				$( this ).closest( 'div' ).find( '._measurement_pricing_weight_enabled' ).change();
			}
		}
	);
	...
	// hide measurement calculator pricing options when product type is variable (WC < 2.0 only)
	$( 'select#product-type' ).change( function() {
		...
		if ( 'variable' == select_val ) {
			$( '#measurement_product_data .hide_if_variable' ).hide();
		}
	} ).change();
