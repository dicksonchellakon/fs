...
class WC_Price_Calculator_Cart {
	...
	public function __construct() {
		...
		if ( version_compare( WOOCOMMERCE_VERSION, "2.0.0" ) >= 0 || WC_Measurement_Price_Calculator::WC_PRE_2_PATCHED ) {
			...
			// WC < 2.0, pricing calculator doesn't support variations due to missing variation_id parameter
		...
		add_action( 'woocommerce_order_item_meta',            array( $this, 'order_item_meta' ),     10, 2 ); // WC < 2.0
		add_action( 'woocommerce_add_order_item_meta',        array( $this, 'add_order_item_meta' ), 10, 2 ); // WC >= 2.0
		...
		add_filter( 'woocommerce_cart_item_price_html',       array( $this, 'get_cart_widget_item_price_html' ), 10, 3 );
	...
	public function add_to_cart_validation( $valid, $product_id, $quantity, $variation_id = '', $variations = array(), $cart_item_data = array() ) {
		...
		$product = WC_Price_Calculator_Product::get_product( $product_id );
		...
		if ( $valid && WC_Price_Calculator_Product::pricing_calculator_enabled( $product ) ) {
			...
			foreach ( $measurements as $measurement ) {
				...
				if ( ! $value || ! is_numeric( $value ) || $value <= 0 ) {
					$woocommerce->add_error( sprintf( __( "%s missing", WC_Measurement_Price_Calculator::TEXT_DOMAIN ), $measurement->get_label() ) );
					$valid = false;
				}
			}
			...
			if ( $settings->pricing_rules_enabled() && $measurement_needed_value ) {
				...
				if ( is_null( $settings->get_pricing_rules_price( $measurement_needed ) ) ) {
					$woocommerce->add_error( __( "No price available for a product with this measurement, please contact the store for assistance.", WC_Measurement_Price_Calculator::TEXT_DOMAIN ) );
					$valid = false;
				}
			}
	 ...
	 */
	public function add_cart_item_data( $cart_item_data, $product_id, $variation_id = null ) {
		...
		if ( WC_Price_Calculator_Product::pricing_calculator_enabled( $product ) ) {
			...
			$_product = $variation_id ? WC_Price_Calculator_Product::get_product( $variation_id, array( 'parent_id' => $product_id ) ) : $product;
			...
			if ( $measurement_needed_value ) {
				...
				if ( $settings->pricing_rules_enabled() ) {
					$_product->price = $settings->get_pricing_rules_price( $measurement_needed );
				}

				// calculate the price
				$price = $_product->get_price() * $measurement_needed->get_value( $settings->get_pricing_unit() );

				// is there a minimum price to use?
				if ( WC_Price_Calculator_Product::get_meta( $product, 'wc_measurement_price_calculator_min_price' ) > $price ) {
					$price = WC_Price_Calculator_Product::get_meta( $product, 'wc_measurement_price_calculator_min_price' );
				}
	...
	/**
	 * Make sure the user-input fields are persisted to the database in the
	 * order item meta.  This is called during the checkout process for each
	 * cart item added to the order.  WooCommerce < 2.0
	 *
	 * @since 3.0
	 * @param order_item_meta $order_item_meta order item meta object
	 * @param array $item array of data representing a cart item
	 */
	public function order_item_meta( $order_item_meta, $item ) {
		...
		if ( isset( $item['pricing_item_meta_data'] ) ) {
			...
			foreach ( $display_data as $name => $value ) {
				$order_item_meta->add( $name, $value );
			}
		}
	...
	public function add_order_item_meta( $item_id, $values ) {
		...
		if ( isset( $values['pricing_item_meta_data'] ) ) {
			...
			woocommerce_add_order_item_meta( $item_id, '_measurement_data', $measurement_data );
			...
			if ( WC_Price_Calculator_Product::pricing_calculator_inventory_enabled( $_product ) && isset( $values['pricing_item_meta_data']['_quantity'] ) ) {
				...
				woocommerce_update_order_item_meta( $item_id, '_qty', $values['pricing_item_meta_data']['_quantity'] );
			}
		}
	}
	...
	public function set_product_prices( $cart_item ) {
		...
		$product = isset( $cart_item['variation_id'] ) && $cart_item['variation_id'] ? WC_Price_Calculator_Product::get_product( $cart_item['product_id'] ) : $cart_item['data'];
		$settings = new WC_Price_Calculator_Settings( $product );
		...
		if ( ! WC_Price_Calculator_Product::pricing_calculator_inventory_enabled( $product ) && isset( $cart_item['pricing_item_meta_data']['_price'] ) ) {
			...
			// is there a minimum price to use?
			if ( WC_Price_Calculator_Product::get_meta( $product, 'wc_measurement_price_calculator_min_price' ) > $cart_item['data']->get_price() * ( $cart_item['quantity'] / $cart_item['pricing_item_meta_data']['_quantity'] ) ) {
				$price = WC_Price_Calculator_Product::get_meta( $product, 'wc_measurement_price_calculator_min_price' );
	...
	public function get_cart_widget_item_price_html( $price_html, $cart_item, $cart_item_key ) {
		...
		// if this is a pricing calculator item
	...
	public function calculate_product_weights( $cart ) {
		...
		foreach ( $cart->cart_contents as $cart_item_key => &$values ) {
			...
			if ( isset( $_product->variation_id ) && $_product->variation_id ) $product = WC_Price_Calculator_Product::get_product( $_product->id );
	...
	public function order_again_cart_item_data( $cart_item_data, $item, $order ) {
		...
		$product = WC_Price_Calculator_Product::get_product( $item['product_id'] );
	...
	public function add_to_cart( $product_id, $measurement_needed, $measurements = array(), $quantity = 1, $variation_id = '', $variation = '', $cart_item_data = array() ) {
		...
		if ( is_null( $woocommerce->cart->cart_contents ) ) $woocommerce->cart->cart_contents = array();

		return $woocommerce->cart->add_to_cart( $product_id, $quantity, $variation_id, $variation, $cart_item_data );
	}
	...
	private function get_measurement_cart_item_data( $item, $cart_item_data ) {
		...
		// always need the actual parent product, not the useless variation product
		$product = isset( $item['variation_id'] ) && $item['variation_id'] ? WC_Price_Calculator_Product::get_product( $item['product_id'] ) : $item['data'];
