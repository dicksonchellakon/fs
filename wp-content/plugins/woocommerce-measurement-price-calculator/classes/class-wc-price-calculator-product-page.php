...
class WC_Price_Calculator_Product_Page {
	...
	public function product_variation_is_visible( $visible, $product_id ) {
		...
		if ( ! $visible && $product->is_type( 'variable' ) && WC_Price_Calculator_Product::pricing_calculator_enabled( $product ) ) {
	...
	public function render_embedded_styles() {
		...
		$product = null;
		if ( is_product() ) $product = WC_Price_Calculator_Product::get_product( $post->ID );
	...
	public function enqueue_frontend_scripts() {
		...
		wp_enqueue_script( 'wc-price-calculator', $wc_measurement_price_calculator->plugin_url() . '/assets/js/price-calculator.js' );
		...
		$wc_price_calculator_params = array(
			...
			'minimum_price'                  => WC_Price_Calculator_Product::get_meta( $product, 'wc_measurement_price_calculator_min_price' ),
		);
	...
	public function render_price_calculator() {
		global $woocommerce, $product, $wc_measurement_price_calculator;
		...
		if ( WC_Price_Calculator_Product::pricing_calculator_enabled( $product ) ) {
			...
			woocommerce_get_template(
				...
				'',
				$wc_measurement_price_calculator->plugin_path() . '/templates/' );
			...
			if ( $product->is_type( 'simple' ) ) {
				...
				$total_price = woocommerce_price( $quantity_range['min_value'] * $product->get_price(), 2 );
