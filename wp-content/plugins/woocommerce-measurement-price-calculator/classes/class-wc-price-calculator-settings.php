...
class WC_Price_Calculator_Settings {
	...
	public function __construct( $product = null ) {
		...
		if ( is_numeric( $product ) ) {
			$product = WC_Price_Calculator_Product::get_product( $product );
		}
		...
		if ( $product ) {
			...
			$settings = WC_Price_Calculator_Product::get_meta( $product, 'wc_price_calculator' );
		}
	 ...
	 *         'area-dimension', 'volume', 'volume-dimension', 'volume-area',
	...
	public function is_calculator_type_derived() {
		return in_array( $this->get_calculator_type(), array( 'area-dimension', 'volume-dimension', 'volume-area', 'wall-dimension' ) );
	}
	...
	public function get_pricing_rules( $to_unit = null ) {
		...
		if ( $this->is_pricing_calculator_enabled() && $this->product ) {
			...
			if ( is_null( $this->pricing_rules ) ) {
				$this->set_pricing_rules( WC_Price_Calculator_Product::get_meta( $this->product, 'wc_price_calculator_pricing_rules' ) );
			}
	...
	public function get_pricing_rule_price_html( $rule ) {
		...
		if ( $rule['price'] > 0 ) {
			...
			if ( '' !== $rule['sale_price'] && '' !== ( $rule['regular_price'] ) ) {
				...
				$price_html .= WC_Price_Calculator_Product::get_price_html_from_to( $rule['regular_price'], $rule['price'], __( $this->get_pricing_label(), WC_Measurement_Price_Calculator::TEXT_DOMAIN ) );

			} else {

				$price_html .= woocommerce_price( $rule['price'] ) . ' ' . __( $this->get_pricing_label(), WC_Measurement_Price_Calculator::TEXT_DOMAIN );
	...
	public function get_options( $measurement_name ) {
		...
		if ( $this->is_pricing_calculator_enabled() && isset( $this->settings[ $calculator_type ][ $measurement_name ]['options'] ) ) {
			foreach ( $this->settings[ $calculator_type ][ $measurement_name ]['options'] as $value ) {
				if ( is_numeric( $value ) ) $options[ $value ] = $value;  // number
	...
	public static function get_measurement_types() {
		return array( 'dimension', 'area', 'area-dimension', 'volume', 'volume-dimension', 'volume-area', 'weight', 'wall-dimension' );
	}
