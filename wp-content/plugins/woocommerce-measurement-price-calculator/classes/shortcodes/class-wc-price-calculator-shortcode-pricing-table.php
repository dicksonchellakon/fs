...
class WC_Price_Calculator_Shortcode_Pricing_Table {
	...
	public static function get( $atts ) {
		global $woocommerce;
		return $woocommerce->shortcode_wrapper( array( __CLASS__, 'output' ), $atts, array( 'class' => 'wc-measurement-price-calculator' ) );
	}
	...
	public static function output( $atts ) {
		global $woocommerce, $wc_pre_orders, $product, $wpdb;
		...
		if ( ! $settings->pricing_rules_enabled() || ! $settings->has_pricing_rules() ) return;
	...
	public static function get_pricing_rules_table( $rules, $settings ) {
		...
		$html .= '<thead><tr><th>' . sprintf( __( 'Range (%s)' ), '<span class="units">' . __( $settings->get_pricing_label(), WC_Measurement_Price_Calculator::TEXT_DOMAIN ) . '</span>' ) . '</th>';
		...
		foreach ( $rules as $rule ) {
			...
			if ( '' === $rule['range_end'] ) $range .= '+';
			elseif ( $rule['range_end'] != $rule['range_start'] ) $range .= ' - ' . $rule['range_end'];
