...
class WC_Price_Calculator_Measurement {
	 ...
	 * @param mixed optional $value the measurement value, int or float.  Defaults to 1
	 ...
	 * measurement units
	 *
	 * @param mixed $value int or float measurement value
	 */
	 ...
	 *
	 * @return mixed the measurement value, int or float
	 */
	public function get_value( $unit = null ) {

		// return in the internal unit
		if ( ! $unit ) return $this->value;
	 ...
	 *
	 * @return numeric the measurement value converted to common units
	 */
	public function get_value_common() {
		return self::convert( $this->value, $this->get_unit_common() );
	}
	 ...
	 *
	 * @return mixed returns the measurement in the new units, int or float
	 */
	 ...
	 *
	 * @return mixed returns the converted measurement value, int or float
	 */
	...
	public static function get_standard_unit( $unit ) {
		...
		if ( isset( $normalize_table[ $unit ]['unit'] ) ) return $normalize_table[ $unit ]['unit'];
		...
		if ( is_null( self::$normalize_table ) ) {
			self::$normalize_table = apply_filters( 'wc_measurement_price_calculator_normalize_table',
				array(
					...
					'ml'    => array( 'factor' => 0.001, 'unit' => 'l' ),
					'cu cm' => array( 'factor' => 0.001, 'unit' => 'l' ),
					'l'     => array( 'factor' => 1,     'unit' => 'l' ),
					'cu m'  => array( 'factor' => 1000,  'unit' => 'l' ),
	...
	public static function get_conversion_table() {
		if ( is_null( self::$conversion_table ) ) {
			self::$conversion_table = apply_filters( 'wc_measurement_price_calculator_conversion_table',
				array(
					...
					'l' => array(
						'ml'      => array( 'factor' => 1000 ),
						'cu cm'   => array( 'factor' => 1000 ),
						'l'       => array( 'factor' => 1 ),
						'cu m'    => array( 'factor' => 0.001 ),
						'fl. oz.' => array( 'factor' => 33.814 ),
						'cup'     => array( 'factor' => 4.22675 ),
						'pt'      => array( 'factor' => 2.11338 ),
						'qt'      => array( 'factor' => 1.05669 ),
						'gal'     => array( 'factor' => 0.264172 ),
						'cu. in.' => array( 'factor' => 61.0237 ),
						'cu. ft.' => array( 'factor' => 0.0353147 ),
						'cu. yd.' => array( 'factor' => 0.00130795062 ),
					),
					'lbs' => array(
						'oz'  => array( 'factor' => 16 ),
						'lbs' => array( 'factor' => 1 ),
