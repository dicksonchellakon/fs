...
class WC_Price_Calculator_Inventory {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_checkout_item_quantity',          array( $this, 'get_checkout_item_quantity' ), 10, 3 );
	...
	public function get_measurement_stock_amount( $quantity, $cart_item_key = null ) {
		...
		if ( ! $this->pricing_stock_altered && isset( $_REQUEST['add-to-cart'] ) && $_REQUEST['add-to-cart'] ) {
			...
			$product = WC_Price_Calculator_Product::get_product( $_REQUEST['add-to-cart'] );
	 ...
	 * @param string $item_id the cart item identifier
	 * @return string the checkout item name html
	 */
	public function get_checkout_item_quantity( $item_quantity_html, $values, $item_id ) {
				...
				if ( $product->get_total_stock() > 0 ) {
					...
					switch ( $format_option ) {
						...
						case 'low_amount' :
							...
							$format = ( $product->get_total_stock() <= $low_amount ) ? __( 'Only %s %s left in stock', 'woocommerce' ) : __( 'In stock', 'woocommerce' );
						break;
						default :
							$format = __( '%s %s in stock', 'woocommerce' );
						break;
					}
	...
	public function get_cart_item_quantity( $quantity_html, $cart_item_key ) {
		...
		if ( WC_Price_Calculator_Product::pricing_calculator_inventory_enabled( $_product ) && isset( $cart[ $cart_item_key ]['pricing_item_meta_data']['_quantity'] ) && $cart[ $cart_item_key ]['pricing_item_meta_data']['_quantity'] ) {
			...
			$quantity_html = sprintf( '<div class="quantity"><input type="number" name="cart[%s][qty]" step="%s" min="%s" max="%s" value="%s" size="4" title="' . _x( 'Qty', 'Product quantity input tooltip', 'woocommerce' ) . '" class="input-text qty text" maxlength="12" /></div>', $cart_item_key, $step, $min, $max, esc_attr( $cart[ $cart_item_key ]['pricing_item_meta_data']['_quantity'] ) );
		}
	...
	public function after_cart_item_quantity_update( $cart_item_key, $quantity ) {
		...
		if ( isset( $cart_items[ $cart_item_key ] ) && $cart_items[ $cart_item_key ] ) {
			...
			$product = WC_Price_Calculator_Product::get_product( $cart_items[ $cart_item_key ]['product_id'] );
			...
			if ( WC_Price_Calculator_Product::pricing_calculator_inventory_enabled( $product ) ) {
				...
				if ( isset( $_REQUEST['quantity'] ) ) {
					...
					// add-to-cart actions
					$woocommerce->cart->cart_contents[ $cart_item_key ]['pricing_item_meta_data']['_quantity'] += $_REQUEST['quantity'];
					...
					if ( isset( $cart_totals[ $cart_item_key ]['qty'] ) && $cart_totals[ $cart_item_key ]['qty'] ) {
						$woocommerce->cart->cart_contents[ $cart_item_key ]['pricing_item_meta_data']['_quantity'] = preg_replace( "/[^0-9\.]/", "", $cart_totals[ $cart_item_key ]['qty'] );
					}
				}
			}
		}
	...
	public function admin_manage_order_stock( $quantity, $item_id ) {
		...
		$product = WC_Price_Calculator_Product::get_product( $order_items[ $item_id ]['product_id'] );
