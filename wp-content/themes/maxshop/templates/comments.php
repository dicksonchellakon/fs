				...
				<div class="media">
					<div class="media-heading">
						...
						<div class="time">
							...
							<time datetime="<?php echo comment_date('c'); ?>"><?php printf(__('%1$s', 'yatheme'), get_comment_date(),  get_comment_time()); ?></time>
						</div>
					</div>
					<?php if ($comment->comment_approved == '0') : ?>
						<div class="awaiting row-fluid">
						  <i><?php _e('Your comment is awaiting moderation.', 'yatheme'); ?></i>
						</div>
					<?php endif; ?>
...
<?php if (post_password_required()) : ?>
	<div id="comments">
		<div class="alert alert-block fade in">
			...
			<p><?php _e('This post is password protected. Enter the password to view comments.', 'yatheme'); ?></p>
		</div>
	</div><!-- /#comments -->
<?php endif; ?>
...
<?php if (have_comments()) : ?>
	...
	<nav id="comments-nav" class="pager">
		...
		<?php else: ?>
			<li class="previous disabled"><a><?php _e('&larr; Older comments', 'yatheme'); ?></a></li>
		<?php endif; ?>
		<?php if (get_next_comments_link()) : ?>
			<li class="next"><?php next_comments_link(__('Newer comments &rarr;', 'yatheme')); ?></li>
		<?php else: ?>
			<li class="next disabled"><a><?php _e('Newer comments &rarr;', 'yatheme'); ?></a></li>
		<?php endif; ?>
		</ul>
	</nav>
...
<div id="respond">
	...
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" name="commentform" onsubmit="return submitform()" class="form-horizontal row-fluid">		
		<?php if (is_user_logged_in()) : ?>
			<p><?php printf(__('Logged in as <a href="%s/wp-admin/profile.php">%s</a>.', 'yatheme'), get_option('siteurl'), $user_identity); ?> <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php __('Log out of this account', 'yatheme'); ?>"><?php _e('Log out &raquo;', 'yatheme'); ?></a></p>
		<?php else : ?>
            <div class="cmm-box-left">
		<div class="control-group">
			<!--<label class="control-label" for="author"><?php _e('Name*','yatheme'); ?></label>-->
			<div class="controls">
		...
		<div class="control-group">			
			...
			<button type="submit" class="btn btn-default">
				<i class="icon-comment-alt"></i><?php _e('Post comment', 'yatheme'); ?>
			</button>
		</div>
        </div>
