...
if (count($list)>0){
?>
<div class="view-all"><a href="<?php echo get_category_link($category) ?>"><?php _e( 'View All', 'yatheme' );?><i class="icon-caret-right"></i></a></div>
	...
	<?php if ( $key == 0 ) {?>
		<li class="widget-post item-<?php echo $key;?> first-news">
			<div class="widget-post-inner">
				...
				<div class="widget-thumb">
					<a href="<?php echo post_permalink($post->ID)?>" title="<?php echo esc_attr( $post->post_title );?>"><?php echo get_the_post_thumbnail($post->ID, 'medium');?></a>
				</div>
				...
				<div class="widget-caption">
					...
					<span class="entry-comment">
						<?php echo '<i class="icon-comment"></i>'.esc_html( $post->comment_count ) ; ?>
					</span>
		...
		<li class="widget-post item-<?php echo $key;?> other-news">
			<div class="widget-post-inner">
					...
					<div class="widget-caption">
					<div class="item-title">
						<h4><a href="<?php echo post_permalink($post->ID)?>" title="<?php echo esc_attr( $post->post_title );?>"><?php echo $post->post_title;?></a></h4>
