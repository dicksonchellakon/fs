...
class YA_Menu {
	...
	function ya_menu_upload(){
		...
		wp_localize_script('ya-menu-upload-js', 'ya_menu_upload', array('url' => ''));
	}
		...
		<li id="menu-item-<?php echo esc_attr( $item_id ); ?>" class="<?php echo implode(' ', $classes ); ?>">
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
				...
				<div class="advance-menu">
					<a href="javascript:void(0);" class="menu-advance-href" data-id="menu-advance-<?php  echo esc_attr( $item_id ) ?>"><?php _e( 'Show/Hide Advance Menu Config', 'yatheme'); ?> </a>
					<div class="menu-config-content" id="menu-advance-<?php  echo $item_id ?>" style="display: none;">
						...
						?><p class="field-<?php echo $field; ?> description description-wide">
								...
								<?php if( $field_opts['type'] !='checkbox' ){ _e( $field_opts['label'], 'yatheme' );} ?>
								<?php
								switch( $field_opts['type'] ):
									...
									?>			
											...
											echo ' <a href="javascript:void(0);" class="ya-menu-upload button-secondary"'.$upload.' rel-id="edit-menu-item-'.esc_attr( $field ).'-'.esc_attr( $item_id ).'">'.__('Browse', 'yatheme').'</a>';
											echo ' <a href="javascript:void(0);" class="ya-menu-upload-remove"'.$remove.' rel-id="edit-menu-item-'.esc_attr( $field ).'-'.esc_attr( $item_id ).'">'.__('Remove', 'yatheme').'</a>';
											?>								
									<?php
										break;	
										case 'checkbox':
				...
				<div class="menu-item-actions description-wide submitbox">
					...
					); ?>"><?php _e('Remove','yatheme'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php	echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
						?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php _e('Cancel','yatheme'); ?></a>
				</div>
