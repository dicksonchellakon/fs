...
class Ya_Portfolio_Shortcode{
	...
	function portfolio( $atts ){
		...
		if( count($categories_id) != 0 ){
			...
			$output .= '<li class="selected" data-portfolio-filter="*">'. __( 'All', 'yatheme' ).' </li>';
			...
			if( $loadmore == 'yes' ){
				...
				$output .= '<span class="des-load" data-label="'.__("Show More Portfolio", "yatheme" ).'" data-label-loaded="'.__("All Items Loaded", "yatheme" ) .'"></span>';		
				$output .= '</div>';
			}
			$output .= '</div>';
		}else{
			$output .= __( 'Please select categories id', 'yatheme' );
		}
		return $output;
	}
