...
class YA_Posts_Widget extends YA_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'ya_posts', 'description' => __('SW Posts', 'yatheme'));
		parent::__construct('ya_posts', __('SW Posts', 'yatheme'), $widget_ops);
...
class YA_Category_List_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_category_list', 'description' =>__('SW Category List header widget', 'yatheme'));
		parent::__construct('ya_category_list', __('SW List Category', 'yatheme'), $widget_ops);
	}
}
class YA_Vertical_Megamenu_Widget extends YA_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'vertical_megamenu', 'description' =>__('SW Vertical Mega Menu Widget', 'yatheme'));
		parent::__construct('vertical_megamenu', __('SW Vertical Mega Menu', 'yatheme'), $widget_ops);
	}
}
...
class YA_Top_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_top', 'description' =>__('SW top header widget', 'yatheme'));
		parent::__construct('ya_top', __('SW Top Widget', 'yatheme'), $widget_ops);
	}
}
...
class YA_Top_Rate_Product extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_top_rate_product woocommerce widget_top_rated_products', 'description' =>__('SW Top Rated Product Widget', 'yatheme'));
		parent::__construct('top_rated_product', __('SW Top Rated Product Widget', 'yatheme'), $widget_ops);
	}
}
...
class YA_Best_Seller_Product extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_best_seller_product', 'description' =>__('SW best seller product woocommerce', 'yatheme'));
		parent::__construct('ya_best_seller_product', __('SW best seller products', 'yatheme'), $widget_ops);
	}
}
...
class YA_Related_Product_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_relate_product', 'description' =>__('SW related products woocommerce', 'yatheme'));
		parent::__construct('ya_relate_product', __('SW related products', 'yatheme'), $widget_ops);
	}
...
class YA_Categories_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_categories', 'description' =>__('SW Categories', 'yatheme'));
		parent::__construct('ya_categories', __('SW Catetories', 'yatheme'), $widget_ops);
	}
}
...
class YA_Feature_Product_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_feature_product', 'description' =>__('SW feature product woocommerce', 'yatheme'));
		parent::__construct('ya_feature_product', __('SW Feature Products', 'yatheme'), $widget_ops);
	}
}
...
class YA_Default_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_default', 'description' =>__('WP Default Widget', 'yatheme'));
		parent::__construct('ya_default', __('WP Default Widget', 'yatheme'), $widget_ops);
	}
}
...
class YA_Social_Widget extends WP_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_social', 'description' => __('SW Social Networks', 'yatheme'));
		parent::__construct('ya_social', __('SW Social', 'yatheme'), $widget_ops);
	...
	function widget($args, $instance){
		...
		<ul>
			...
			<?php preg_match('/icon-.*?/', $social['icon'], $match); ?>
			<li><a href="<?php echo esc_url( $social['link'] ); ?>"
				title="<?php if (empty($match)) echo esc_attr( $social['icon'] ); ?>"> <?php if (empty($match)) echo esc_html( $social['icon'] ); else { ?>
					<i class="<?php echo esc_attr( $social['icon'] )?>"></i> <?php }?>
			</a></li>
			<?php } ?>
		</ul>
		<?php
	...
	function form($instance){ //var_dump($instance);
		...
		?>
<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title: ', 'yatheme'); ?>
...
<?php //var_dump($socials);
		...
		foreach ($socials as $j => $social){
			...
			?>
<p>
	<strong>Social <?php echo $j+1;?>
	</strong><br>
	<?php if (key_exists('icon', $social)){?>
	<label for="<?php echo esc_attr( $id_j_icon ); ?>"><?php _e('icon-* | title', 'yatheme')?>
	...
	<?php }?>
	<label for="<?php echoesc_attr( $id_j_link ); ?>"><?php _e('Link ', 'yatheme')?> </label>
...
<?php
		...
		?>
<p>
	<strong>Add a new social network</strong><br> <label
		for="<?php echo esc_attr( $id_i_icon ); ?>"><?php _e('Classname for Icon or Title', 'yatheme'); ?>
	</label> <input class="widefat" id="<?php echo esc_attr( $id_i_icon ); ?>"
		name="<?php echo $name_i_icon; ?>" type="text" value=""
		placeholder="Enter Font Awesome icon or Title" /> <span>If using as
		icon, please choose class name in Font Awesome. This is required.</span>
	<label for="<?php echo esc_attr( $id_i_link ); ?>"><?php _e('Link ', 'yatheme')?> </label>
...
class YA_Cpanel extends WP_Widget{
	...
	public function __construct(){
		$widget_opts = array( 'classname' => 'cpanel', 'description' => __('Theme Options on Frontend', 'yatheme') );
		parent::__construct('cpanel', __('YA cPanel', 'yatheme'), $widget_opts);
	}
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo $widget->get_field_id('widget_style'); ?>"><?php _e( 'Widget Style', 'yatheme' ) ?>
...
//		return $this->menu_assignments[$hash];
	...
	private function getDisplaySelectOptions(){
		if ( is_null($this->display_select_tpl) ){
			$this->display_select_options = $display_select_options = array(
				'all' => __('All', 'yatheme'),
				'if_selected' => __('Only on selected', 'yatheme'),
				'if_no_selected' => __('Except selected', 'yatheme')
			);
			$html = '';
			$html .= '<p>';
			$html .= '<label for="id-display-select">'. __( 'Widget Display', 'yatheme' ) .  '</label>';
	...
	private function getDisplayCheckboxUsers(){
		if ( is_null($this->display_checkbox_users_tpl) ){
			$this->display_checkbox_users = $display_checkbox_users = array(
				'login' => __('Show only for login users' , 'yatheme'),
				'logout' => __('Show only for logout users' , 'yatheme')
			);
			...
			foreach ($display_checkbox_users as $key => $name) {
				...
				$html .= '<label for="id-display-checkbox-user-' . $key . '">
						<input type="checkbox" id="id-display-checkbox-user-' . $key . '"
								name="name-display-checkbox-user-' . $key . '" checked-display-checkbox-user-'. $key .'/> ' . __( $name, 'yatheme' ) . 
					'</label> <br/>';
			}
			$html .= '</p>';
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. __( 'Language Display', 'yatheme' ) .  '</label>';
	...
	private function getDisplayCheckboxGeneral(){
		if ( is_null($this->display_checkbox_general_tpl) ){
			$this->display_checkbox_general = $display_checkbox_general = array(
					'home' => __('Blog posts', 'yatheme'),
					'archive' => __('Archive page', 'yatheme'),
					'date' => __('Archive Date', 'yatheme'),
					'category' => __('Category archive', 'yatheme'),
					'tag' => __('Tag archive', 'yatheme'),
					'author' => __('Author archive', 'yatheme'),
					'search' => __('Search page', 'yatheme'),
					'single' => __('Single post', 'yatheme'),
					'page' => __('Page template', 'yatheme'),
					'404' => __('Page 404', 'yatheme')
				);
			$html = '';
	...
	private function render_wdisplayxx($widget_display){
		...
		$display_checkbox['general'] = array(
					'front_page' => __('Front page', 'yatheme'),
					'home' => __('Blog posts', 'yatheme'),
					'archive' => __('Archive page', 'yatheme'),
					'category' => __('Category archive', 'yatheme'),
					'tag' => __('Tag archive', 'yatheme'),
					'author' => __('Author archive', 'yatheme'),
					'search' => __('Search page', 'yatheme'),
					'page' => __('Pages', 'yatheme'),
					'single' => __('Single post page', 'yatheme')
				);
		//Get pages
		$pages = get_pages();
		...
		<p>
			<label
				for="<?php echo $this->widget->get_field_id('display_select'); ?>"><?php _e( 'Widget Display', 'yatheme' ) ?>
			</label>
		...
		foreach ($display_checkbox['general'] as $value => $name) { ?>
			...
			<input type="checkbox" id="<?php $this->widget->get_field_id($value)?>"
				...
				/>
			<label for="<?php echo $this->widget->get_field_id($value); ?>"><?php _e( $name) ?></label><br/>
		<?php } ?>
		</p>
