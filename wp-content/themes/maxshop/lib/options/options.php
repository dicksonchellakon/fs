...
if ( ! class_exists('YA_Options') ){
	...
	class YA_Options{
		...
		public function __construct($sections = array(), $args = array()){
			...
			$defaults['page_title'] = __('Options', 'yatheme');
			...
			//$defaults['help_sidebar'] = __('', 'yatheme');
...
// 					);
		...
		function _enqueue(){
			...
			wp_localize_script('ya-opts-js', 'ya_opts', array('reset_confirm' => __('Are you sure? Resetting will loose all custom values.', 'yatheme'), 'opt_name' => $this->args['opt_name']));
		...
		public function _options_form(){
			...
			<div class="cpannel-button">
				<button id="cpanel-submit" class="btn btn-inverse" type="submit"><?php _e( 'SAVE', 'yatheme' ); ?></button>
				<button id="cpanel-reset" class="btn btn-inverse" type="button"><?php _e( 'RESET', 'yatheme' ); ?></button>
			</div>
			<?php
			echo '</div>';
		...
		function _options_page_html2(){
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="ya-opts-form-wrapper">';
			settings_fields($this->args['opt_name'].'_group');
			...
			submit_button(__('Reset to Defaults', 'yatheme'), 'secondary', $this->args['opt_name'].'[defaults]', false);
			...
			if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('ya-opts-saved') == '1'){
				if(isset($this->options['imported']) && $this->options['imported'] == 1){
					echo '<div id="ya-opts-imported">'.apply_filters('ya-opts-imported-text-'.$this->args['opt_name'], __('<strong>Settings Imported!</strong>', 'yatheme')).'</div>';
				}else{
					echo '<div id="ya-opts-save">'.apply_filters('ya-opts-saved-text-'.$this->args['opt_name'], __('<strong>Settings Saved!</strong>', 'yatheme')).'</div>';
				}
				delete_transient('ya-opts-saved');
			}
			echo '<div id="ya-opts-save-warn">'.apply_filters('ya-opts-changed-text-'.$this->args['opt_name'], __('<strong>Settings have changed!, you should save them!</strong>', 'yatheme')).'</div>';
			echo '<div id="ya-opts-field-errors">'.__('<strong><span></span> error(s) were found!</strong>', 'yatheme').'</div>';

			echo '<div id="ya-opts-field-warnings">'.__('<strong><span></span> warning(s) were found!</strong>', 'yatheme').'</div>';
...
// 				echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="ya-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'img/glyphicons/glyphicons_082_roundabout.png" /> <span>'.__('Import / Export', 'yatheme').'</span></a>';
// 				echo '</li>';
// 				// echo '<li class="divide">&nbsp;</li>';
// 			}//if
...
// 			}
			...
			if(true === $this->args['dev_mode']){
				...
				echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="ya-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> <span>'.__('Dev Mode Info', 'yatheme').'</span></a>';
				echo '</li>';
			}//if
...
// 				echo '<h3>'.__('Import / Export Options', 'yatheme').'</h3>';

// 				echo '<h4>'.__('Import Options', 'yatheme').'</h4>';
...
// 				echo '<p class="description" id="import-code-description">'.apply_filters('ya-opts-import-file-description',__('Input your backup file below and hit Import to restore your sites options from a backup.', 'yatheme')).'</p>';
...
// 				echo '<p class="description" id="import-link-description">'.apply_filters('ya-opts-import-link-description',__('Input the URL to another sites options set and hit Import to load the options from that site.', 'yatheme')).'</p>';
...
// 				echo '<p id="ya-opts-import-action"><input type="submit" id="ya-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.__('Import', 'yatheme').'"> <span>'.apply_filters('ya-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'yatheme')).'</span></p>';
// 				echo '<div id="import_divide"></div>';

// 				echo '<h4>'.__('Export Options', 'yatheme').'</h4>';
// 				echo '<div class="ya-opts-section-desc">';
...
// 			}
			...
			if(true === $this->args['dev_mode']){
				...
				echo '<h3>'.__('Dev Mode Info', 'yatheme').'</h3>';
				echo '<div class="ya-opts-section-desc">';
